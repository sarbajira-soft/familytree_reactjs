...
const ProductDetail = ({ product, isOpen, onClose, onAddToCart, mode = 'modal' }) => {
  ...
  const priceAmount = useMemo(() => getVariantPriceAmount(currentVariant), [currentVariant]);
        ...
        <div className="flex flex-col gap-4 rounded-xl bg-white p-4 shadow-sm">
          ...
          <div className="flex items-center justify-between gap-3 pt-2">
            ...
            <button
              ...
              className={`inline-flex flex-1 items-center justify-center gap-2 rounded-xl px-4 py-3 sm:px-5 sm:py-3 text-xs sm:text-sm font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed ${
                inStock
                  ? 'bg-gradient-to-r from-orange-500 to-orange-600 text-white shadow-md hover:shadow-lg active:scale-95'
                  : 'bg-gray-100 text-gray-400 cursor-not-allowed'
              }`}
            >
              {isAdding ? (
                <>
                  <svg className="animate-spin h-4 w-4 sm:h-4 sm:w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <>
                  <FiShoppingCart size={16} className="sm:size-18" />
