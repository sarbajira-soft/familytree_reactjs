...
import { FiCheck, FiFilm, FiMessageCircle, FiSearch, FiUsers, FiX } from 'react-icons/fi';
...
const ChatShareTargetModal = ({ currentUserId, isOpen, onClose, shareItem }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [conversationResponse, roomResponse, contactResponse] = await Promise.all([
          getConversations(),
          getRooms(),
          getFamilyMembersForChat(),
        ]);
        ...
        const conversationTargets = (conversationResponse?.conversations || []).map((conversation) => ({
          ...
          avatarUrl: conversation?.participants?.find(
            (participant) => Number(participant?.userId || 0) !== Number(currentUserId || 0),
          )?.profileUrl || '',
          section: 'Recent chats',
        }));

        const roomTargets = (roomResponse?.rooms || []).map((room) => ({
          key: `room:${room.id}`,
          targetType: 'room',
          id: Number(room.id),
        ...
        const contactTargets = (contactResponse?.contacts || []).map((contact) => ({
        ...
        setTargets([...conversationTargets, ...roomTargets, ...contactTargets]);
      } catch (loadError) {
        setError(loadError?.message || 'Unable to load chat targets right now.');
      } finally {
        if (!cancelled) {
          setLoading(false);
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const results = await Promise.allSettled(
        selectedTargets.map(async (target) => {
          ...
          if (target.targetType === 'contact') {
            const conversation = await createConversation(familyCode, Number(target.id));
      ...
      if (successCount > 0) {
        toast.success(
          successCount === 1 ? 'Shared to chat' : `Shared to ${successCount} chats`,
        );
      }
      if (failedCount > 0) {
        toast.error(
  ...
  const isVideoPreview = String(shareItem?.previewMediaKind || '').trim().toLowerCase() === 'video';
  ...
  return (
    ...
    >
      <div
        className="flex max-h-[84vh] w-full max-w-[1080px] flex-col overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-2xl"
        onClick={(event) => event.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-label="Share to chat"
      >
        <div className="flex items-start justify-between border-b border-gray-100 px-4 pb-2.5 pt-3 sm:px-5">
          <div className="min-w-0">
            <h3 className="text-[15px] font-semibold text-gray-900 sm:text-base">Share to chat</h3>
        ...
        <div className="flex min-h-0 flex-1 flex-col md:flex-row">
          <div className="border-b border-gray-100 px-4 py-3 md:w-[340px] md:shrink-0 md:border-b-0 md:border-r md:px-5 md:py-4">
            <div className="rounded-3xl bg-slate-50 p-3">
              <div className="flex items-start gap-3">
                {shareItem?.previewMediaUrl && isVideoPreview ? (
                  <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-2xl bg-slate-900">
                    <video
                      ...
                      playsInline
                      preload="metadata"
                      aria-label={shareItem.previewTitle || 'Shared content'}
                    />
                    <span className="absolute bottom-1.5 left-1.5 inline-flex items-center gap-1 rounded-full bg-black/65 px-1.5 py-0.5 text-[10px] font-semibold text-white">
                      <FiFilm size={10} /> Video
                    </span>
                  </div>
                ) : shareItem?.previewMediaUrl ? (
                  <img
                    src={shareItem.previewMediaUrl}
                  ...
                  <div className="grid h-16 w-16 shrink-0 place-items-center rounded-2xl bg-white text-slate-500">
                    {shareItem?.shareType === 'gallery' ? <FiUsers size={18} /> : <FiMessageCircle size={18} />}
                  </div>
                )}
                <div className="min-w-0 flex-1">
                  ...
                  {shareItem?.previewText ? (
                    <div className="mt-1 line-clamp-3 text-xs leading-5 text-gray-500">
                      {shareItem.previewText}
                    </div>
                  ) : null}
                </div>
            ...
            <div className="mt-4">
              ...
              <textarea
                ...
                className="min-h-[140px] w-full rounded-3xl border border-gray-200 px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-200"
              />
            </div>
          </div>
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="border-b border-gray-100 px-4 py-3 md:px-5">
              <label className="flex items-center gap-3 rounded-2xl border border-gray-200 px-4 py-2.5">
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 md:px-4">
              {loading ? (
                <div className="py-6 text-center text-sm text-gray-500">Loading chat targets...</div>
              ) : groupedTargets.length === 0 ? (
                <div className="py-6 text-center text-sm text-gray-500">No chats or contacts found.</div>
              ) : (
                groupedTargets.map(([sectionTitle, sectionTargets]) => (
                  <div key={sectionTitle} className="mb-3 last:mb-0">
                    <div className="px-2 pb-1.5 text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                      {sectionTitle}
                    </div>
        ...
        <div className="flex gap-3 border-t border-gray-200 px-4 py-2.5 sm:px-5">
          <button
            type="button"
            onClick={() => onClose?.()}
            disabled={submitBusy}
