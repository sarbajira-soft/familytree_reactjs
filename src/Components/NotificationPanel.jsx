...
const NotificationPanel = ({
  ...
  const notificationTypes = {
    ...
    TREE_LINK_REQUEST: { icon: <FiLink />, color: "from-blue-500 to-blue-300" },
  ...
  const fetchNotifications = async (getAll = false) => {
    try {
      ...
      const response = await authFetchResponse(url, { method: "GET" });
      ...
      const associationNotifs = formatted.filter(
        (n) =>
      ...
      associationNotifs.forEach((notif) => {
        console.log(
          `🔍 Association Notification ${notif.id}: status=${notif.status}, read=${notif.read}, isRead=${notif.isRead}`,
        );
      });
  ...
  const markAsRead = async (id) => {
    try {
      await authFetchResponse(`/notifications/${id}/read`, {
        method: "POST",
      });
  ...
  const allAssociationRequests = notifications.filter(
    (n) =>
      n.type === "family_association_request" ||
      n.type === "FAMILY_ASSOCIATION_REQUEST",
  );

  const allTreeLinkRequests = notifications.filter(
    (n) => n.type === "TREE_LINK_REQUEST",
  );

  const totalRequestCount =
    allAssociationRequests.length + allTreeLinkRequests.length;

  const allOtherNotifications = notifications.filter(
    (n) =>
      n.type !== "family_association_request" &&
      n.type !== "FAMILY_ASSOCIATION_REQUEST" &&
      n.type !== "TREE_LINK_REQUEST",
  );
  ...
  const getFilteredNotifications = () => {
    switch (activeTab) {
      ...
      case "other":
        ...
        return {
          associationRequests: [],
          otherNotifications: allOtherNotifications,
        };
  ...
  const { associationRequests, otherNotifications } =
  ...
  console.log("📊 Time-based filtering:", {
    ...
    last7Days:
      associationGroups.last7Days.length + otherGroups.last7Days.length,

    last30Days:
      associationGroups.last30Days.length + otherGroups.last30Days.length,
  });
  ...
  const handleAcceptRequest = async (notification) => {
    try {
      ...
      const isTreeLink = notification.type === "TREE_LINK_REQUEST";
      const url = isTreeLink
        ? `${import.meta.env.VITE_API_BASE_URL}/notifications/respond`
        : `${import.meta.env.VITE_API_BASE_URL}/family/accept-association`;

      const body = isTreeLink
        ? { notificationId: notification.id, action: "accept" }
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to accept request: ${errorData.message || "Unknown error"}`,
        );
  ...
  const handleRejectRequest = async (notification) => {
    try {
      ...
      const url = isTreeLink
        ? `${import.meta.env.VITE_API_BASE_URL}/notifications/respond`
        : `${import.meta.env.VITE_API_BASE_URL}/family/reject-association`;

      const body = isTreeLink
        ? { notificationId: notification.id, action: "reject" }
        : { requestId: notification.id };
      ...
      if (response.ok) {
        ...
        alert(
          `Failed to reject request: ${errorData.message || "Unknown error"}`,
        );
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="border-b border-gray-200 bg-white px-4 py-3 dark:bg-slate-900 dark:border-slate-800">
            ...
            <div className="flex gap-2 p-2">
              ...
              >
                Requests
                <span className="ml-1 text-xs">({totalRequestCount})</span>
              </button>
                ...
                {(associationGroups.today.length > 0 ||
                  ...
                  <>
                    ...
                    {associationGroups.today.map((notification) => {
                      const isTreeLink =
                        notification.type === "TREE_LINK_REQUEST";
                      ...
                      const treeLinkRel = String(
                        notification.data?.relationshipType || "",
                      ).trim();
                      const treeLinkMessage = treeLinkRel
                        ? `Tree link request (${treeLinkRel})`
                        : "Tree link request";
