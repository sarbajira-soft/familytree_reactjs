...
const getGenderLabel = (person, tree, currentUserId) => {
  ...
  const isSpouseOfCurrentUser = () => {
    ...
    const spouses =
      currentUser.spouses instanceof Set
...
const Person = ({
  ...
  const isLinkedMode = urlMode === "linked";
  ...
  const handleSaveLabel = async (e) => {
    ...
    if (!currentUserId || !currentFamilyId) {
      Swal.fire({
        icon: "warning",
        title: "Missing info",
        text: "User ID or Family Code missing. Cannot save label.",
      });
      return;
    }
  ...
  const getAssociatedCodes = () => {
    ...
    if (Array.isArray(person.associatedFamilyCodes)) {
      codes = person.associatedFamilyCodes.filter(
        (code) => code && !code.startsWith("REL_"),
      );
    } else if (
      typeof person.associatedFamilyCodes === "string" &&
      person.associatedFamilyCodes
    ) {
      try {
        const arr = JSON.parse(person.associatedFamilyCodes);

        if (Array.isArray(arr))
          codes = arr.filter((code) => code && !code.startsWith("REL_"));
      } catch {
        if (!person.associatedFamilyCodes.startsWith("REL_"))
          codes = [person.associatedFamilyCodes];
      }
    }
  ...
  const getNavigationTargetFamilyCode = () => {
    ...
    const personBirthFamilyCode =
      person.primaryFamilyCode || person.familyCode || "";

    // Prefer explicit birth family if it differs from the currently viewed family tree

    if (
      personBirthFamilyCode &&
      personBirthFamilyCode !== currentViewFamilyCode
    ) {
  ...
  const currentViewFamilyCode = code || userInfo?.familyCode || "";
  const rawCanonicalFamilyCode = person?.canonicalFamilyCode
  ...
  const showAssociatedTreeIcon =
    !showLinkedTreeIcon && !!associatedTargetFamilyCode;
  ...
  const handleViewPersonBirthFamily = (e) => {
    ...
    if (
      relationshipCode &&
      (relationshipCode.endsWith("H") || relationshipCode.endsWith("W"))
    ) {
  ...
  const handleViewPersonLinkedFamily = (e) => {
    e.stopPropagation();
    ...
    if (!personFamilyCode) {
      Swal.fire({
        ...
        title: "No Linked Family Tree Found",

        text: "This member does not have a linked family tree available.",
    ...
    if (code === personFamilyCode) {
      Swal.fire({
        ...
        text: `You are already viewing ${person.name}'s linked family tree.`,
  ...
  return (
    ...
    >
      ...
      >
        ...
        {(showLinkedTreeIcon || showAssociatedTreeIcon) && (
