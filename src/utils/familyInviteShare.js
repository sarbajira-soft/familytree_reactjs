...
export function buildMemberInviteLink(familyCode, memberId, rawBaseUrl) {
  const normalizedFamilyCode = String(familyCode || '').trim().toUpperCase();
  if (!normalizedFamilyCode) {
    throw new Error('Family code is required');
  }
...
export function buildMemberInviteMessage(familyCode, signUpLink, personName = 'family member') {
  const normalizedFamilyCode = String(familyCode || '').trim().toUpperCase();
  return [
    `Join our family tree on the app!`,
    `Family Code: ${normalizedFamilyCode}`,
    `Sign up here: ${signUpLink}`,
  ].join('\n');
}

export function getMemberInviteContent(familyCode, memberId, personName, rawBaseUrl) {
  const signUpLink = buildMemberInviteLink(familyCode, memberId, rawBaseUrl);
  return {
    familyCode: String(familyCode || '').trim().toUpperCase(),
    signUpLink,
    message: buildMemberInviteMessage(familyCode, signUpLink, personName),
  };
}

export async function copyMemberInvite(familyCode, memberId, personName, options = {}) {
  const { message, signUpLink } = getMemberInviteContent(familyCode, memberId, personName, options.baseUrl);
  if (!navigator?.clipboard?.writeText) {
    throw new Error('Clipboard sharing is not available on this device.');
  }
  await navigator.clipboard.writeText(message);
...
export async function shareMemberInvite(familyCode, memberId, personName, options = {}) {
  const { title = 'Complete your profile on the app!', baseUrl } = options;
  const { familyCode: normalizedFamilyCode, message, signUpLink } = getMemberInviteContent(familyCode, memberId, personName, baseUrl);

  if (navigator?.share) {
    await navigator.share({
      title,
      text: `Join our family tree and complete your profile!\nFamily Code: ${normalizedFamilyCode}`,
      url: signUpLink,
    });
    return { method: 'native', message, signUpLink };
  }

  return await copyMemberInvite(normalizedFamilyCode, memberId, personName, { baseUrl });
}
