...
const PostPage = () => {
  ...
  const toggleLike = async (postId) => {
    ...
    // Get current post state for potential rollback
    const targetPost = posts.find((p) => p.id === postId);
    if (!targetPost) return;

    const originalLiked = targetPost.liked;
    const originalLikes = targetPost.likes;

    // 🟢 Optimistic update - show like immediately
    setPosts((prevPosts) =>
      prevPosts.map((post) =>
        post.id === postId
          ? {
    ...
    try {
      ...
      if (data) {
        // Update with server response (confirm the optimistic update)
        setPosts((prevPosts) =>
          prevPosts.map((post) =>
            post.id === postId
        ...
        setPosts((prevPosts) =>
          prevPosts.map((post) =>
            post.id === postId
              ? { ...post, liked: originalLiked, likes: originalLikes }
              : post
          )
        );
      }
                ...
                {replyingToCommentId === cmt.id && (
                  <div className="mt-2">
                    <input
                      ...
                      className="w-full dark:bg-slate-800 dark:text-white border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                    />
                    <div className="flex gap-2 mt-2">
                      <button
